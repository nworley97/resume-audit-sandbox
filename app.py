...
# ─────────── AI helpers (JSON extract + fit score) ────────────────
  ...
  <div class="container-fluid">
    ...
    <div>
      {% if current_user.is_authenticated %}
        ...
        <a class="btn btn-outline-secondary btn-sm me-2" href="{{ url_for('edit_jd') }}">Edit JD</a>
...
def edit_jd():
    ...
    return page("Edit JD", body)

def current_jd():
    with SessionLocal() as db:
        return db.get(JobDescription, 1)

# ─────────────── Upload résumé & score route ─────────────────────
...
def home():
    ...
    if request.method == "POST":
        ...
        if not name or not f.filename:
            flash("Name & file required"); return redirect("/")
        ...
        try:
            ...
            flash("Upload PDF or DOCX"); return redirect("/")
        rjs = resume_json(text)
        score = fit_score(rjs, jd_text)
        cid = str(uuid.uuid4())[:8]
        storage_path = upload_pdf(path)
        with SessionLocal() as db:
            db.add(Candidate(
                id=cid, name=name,
    ...
    return page("Upload Résumé", body)
...
def recruiter():
    ...
    table = "".join(
        f"<tr>"
        f"<td>{c.jd_code}</td><td>{c.name}</td><td>{c.fit_score}</td>"
